...
import { consumeRateLimit, hashClientIdentifier, isHoneypotTriggered } from "../../src/security.js";
import { createRouteContext } from "../helpers/mock-plugin.js";
...
describe("security helpers", () => {
	...
	it("expires stale rate-limit state automatically", async () => {
		...
		const { ctx, kv } = createRouteContext({
			input: {},
			kv: {
		...
		const result = await consumeRateLimit(ctx, {
			formId: "form-1",
			ip: "203.0.113.10",
	...
	it("enforces best-effort rate limits", async () => {
		...
		const first = await consumeRateLimit(ctx, {
			formId: "form-1",
			ip: "203.0.113.10",
			now: new Date("2026-04-03T00:00:00.000Z"),
		});
		const second = await consumeRateLimit(ctx, {
			formId: "form-1",
			ip: "203.0.113.10",
			now: new Date("2026-04-03T00:10:00.000Z"),
		});
