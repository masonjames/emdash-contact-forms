...
import { formatCsv, formatSubmissionText, resolveNotificationRecipients } from "../../src/format.js";
...
describe("format helpers", () => {
	it("falls back to plugin-level notification recipient", () => {
		...
		expect(resolveNotificationRecipients(["team@example.com", "team@example.com"], "ops@example.com")).toEqual([
			"team@example.com",
		]);
	});
	...
	it("formats submissions for plain-text email", () => {
		...
		const text = formatSubmissionText(form, {
			name: "Ada",
