...
class MemoryCollection<T> implements StorageCollection<T> {
	...
	constructor(seed: Array<{ id: string; data: T }> = []) {
		for (const item of seed) {
			this.items.set(item.id, item.data);
		}
	}
	...
	async deleteMany(ids: string[]): Promise<number> {
		let count = 0;
		for (const id of ids) {
			if (this.items.delete(id)) count += 1;
		}
		return count;
	}
	...
	async query(options: {
		...
		if (options.where) {
			rows = rows.filter((row) => matchesWhere(row.data, options.where!));
		}

		if (orderEntries.length > 0) {
			rows.sort((a, b) => compareByOrder(a.data, b.data, orderEntries));
		}
...
export function createRouteContext<TInput>(options: {
	input: TInput;
	...
	const ctx = {
		input: options.input,
		request: new Request(options.requestUrl ?? "https://example.com/_emdash/api/plugins/masonjames-contact-forms/test"),
		requestMeta: {
			ip: "203.0.113.10",
			userAgent: "Vitest",
			referer: "https://example.com/contact",
		...
		email: {
			send: async (message: { to: string; subject: string; text?: string; html?: string }) => {
				sent.push(message);
				if (options.emailSend) {
					await options.emailSend(message);
				}
			},
		},
		log: createLogger(),
...
function compareByOrder(
	a: unknown,
	b: unknown,
	orderEntries: Array<[string, "asc" | "desc"]>,
) {
