...
import type { FormPage } from "../../src/domain/types.js";
import {
	coerceCheckboxValue,
	normalizeFieldName,
	normalizeSlug,
...
describe("validation helpers", () => {
	...
	it("rejects invalid form definitions", () => {
		...
		const result = validateFormDefinition({ pages, settings: { redirectUrl: "javascript:alert(1)" } });
	...
	it("validates submissions against declared fields only", () => {
		...
		expect(result.data).toEqual({ name: "Ada", email: "ada@example.com", reason: "sales", consent: true });
		expect("unexpected" in result.data).toBe(false);
	});

	it("rejects missing required text fields", () => {
		const fields = [{ id: "1", type: "text", label: "Name", name: "name", required: true }] as const;
		const result = validateSubmission(fields as never, { name: "" });
		expect(result.valid).toBe(false);
		expect(result.errors).toEqual([{ field: "name", message: "Name is required" }]);
	});

	it("rejects invalid select values", () => {
		const fields = [{ id: "1", type: "select", label: "Reason", name: "reason", required: true, options: [{ label: "Sales", value: "sales" }] }] as const;
		const result = validateSubmission(fields as never, { reason: "support" });
		expect(result.valid).toBe(false);
	...
	it("reports invalid email and missing required checkbox", () => {
		...
		const result = validateSubmission(fields as never, { email: "not-an-email" });
		expect(result.valid).toBe(false);
